...
export async function POST(request: Request) {
  try {
    const body = (await request.json()) as ProjectEnquiry;
    ...
    const companyFax = clean(body.companyFax, 200);

    const websiteType = clean(
      body.websiteType || "Ready-Made Website",
      100,
    );
    ...
    /* =========================================================
       ...
       Return success so they do not know they were blocked.
    ========================================================= */
    if (companyFax) {
    ...
    ========================================================= */
    const subject =
      `New Cleaning Website Enquiry — ${business}`;
    ...
    const emailText = [
      ...
      "",
      "--------------------------------",
      "Submitted through the SparkleClean",
    ...
    const { data, error } =
      await resend.emails.send({
        ...
        /*
          When you receive the email, replying should go
          to the customer's submitted email address.
        */
        replyTo: email,
      });
    ...
    console.error(
      "Cleaning project enquiry error:",
      error,
    );
